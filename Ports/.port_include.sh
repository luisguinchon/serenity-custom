...
get_new_config_guess() {
    ...
    if ! run grep -q LXsystem "$config_guess"; then
...
ensure_build() {
    ...
    if [ ! -f "${DESTDIR}/usr/lib/libc.so" ]; then
        echo "libc.so could not be found. This likely means that LXsystem:"
...
do_generate_patch_readme() {
    ...
    echo "# Patches for $port on LXsystem" > ReadMe.md
    echo >> ReadMe.md
