...
ErrorOr<int> serenity_main(Main::Arguments)
{
    ...
    // -----------------------------------------------------
    // LXsystem custom wallpaper and logo defaults
    // -----------------------------------------------------
    auto theme_name = WindowServer::g_config->read_entry("Theme", "Name", "Default");
    auto wallpaper_path = WindowServer::g_config->read_entry("Background", "Wallpaper", "/res/wallpapers/1.png");

    WindowServer::g_config->write_entry("Background", "Wallpaper", wallpaper_path);
    WindowServer::g_config->write_entry("Logo", "Path", "/res/logo/logo.png");
    WindowServer::g_config->sync();
    // -----------------------------------------------------
    ...
    dbgln("Entering LXsystem WindowServer main loop");
    loop.exec();
    VERIFY_NOT_REACHED();
}
