/*
 * Copyright (c) 2020-2022, the LXsystem developers.
...
ErrorOr<void> ChessWidget::export_pgn(Core::File& file) const
{
    ...
    TRY(file.write_until_depleted("[Site \"LXsystem Chess\"]\n"sv.bytes()));
    ...
    auto const player2 = (!m_engine.is_null() ? "LXsystem ChessEngine"sv : "?"sv);
    ...
    TRY(file.write_until_depleted("[Annotator \"LXsystem Chess\"]\n"sv.bytes()));
