...
function(serenity_install_sources)
    cmake_path(RELATIVE_PATH CMAKE_CURRENT_SOURCE_DIR BASE_DIRECTORY ${LXsystem_SOURCE_DIR} OUTPUT_VARIABLE current_source_dir_relative)
...
function(serenity_app target_name)
    ...
    set(small_icon "${LXsystem_SOURCE_DIR}/Base/res/icons/16x16/${SERENITY_APP_ICON}.png")
    set(medium_icon "${LXsystem_SOURCE_DIR}/Base/res/icons/32x32/${SERENITY_APP_ICON}.png")
...
function(embed_resource target section file)
    ...
    add_custom_command(
        OUTPUT "${asm_file}"
        COMMAND "${LXsystem_SOURCE_DIR}/Meta/generate-embedded-resource-assembly.sh" "${asm_file}" "${section}" "${input_file}" "${file_size}"
        DEPENDS "${input_file}" "${LXsystem_SOURCE_DIR}/Meta/generate-embedded-resource-assembly.sh"
...
function(download_file_multisource urls path)
    ...
    foreach(url ${urls})
        set(DOWNLOAD_COMMAND "${Python3_EXECUTABLE}" "${LXsystem_SOURCE_DIR}/Meta/download_file.py" "-o" "${path}" "${url}")
