...
# Copyright (c) 2018-2023, the LXsystem developers.
...
@unique
class Arch(Enum):
    """LXsystem architecture, not host architecture."""
...
def determine_qemu_kind() -> QEMUKind:
    ...
    if os.uname().sysname == "LXsystem":
        return QEMUKind.LXsystem
...
def determine_serenity_arch() -> Arch:
    ...
    if arch == "x86_64":
        return Arch.x86_64
    raise RunError("Please specify a valid LXsystem architecture")
...
def determine_machine_type() -> MachineType:
    ...
    if provided_machine_type is not None:
        try:
            ...
            raise RunError(f"{provided_machine_type} is not a valid LXsystem machine type")
        return value
...
def determine_boot_drive_type() -> BootDriveType:
    ...
    if provided_boot_drive_type is not None:
        try:
            ...
            raise RunError(f"{provided_boot_drive_type} is not a valid LXsystem boot drive type")
        return value
...
def set_up_screens(config: Configuration):
    ...
    if provided_display_backend is not None:
        ...
        if len(config.spice_arguments) != 0:
            ...
            # https://github.com/LXsystem/serenity/issues/7657
        ...
        elif config.qemu_kind == QEMUKind.LXsystem:
            config.display_backend = "sdl,gl=off"
        else:
...
def set_up_machine_devices(config: Configuration):
    ...
    config.extra_arguments.extend(["-name", "LXsystem", "-d", "guest_errors"])
