...
for md_file in $(find "${MAN_DIR}" -iname '*.md' | ${SORT}); do
    ...
    pandoc -f gfm -t html5 -s \
        ...
        --metadata title="${name}(${section_number}) - LXsystem man pages" \
        -o "${output_file}" \
        "${md_file}" &
done
...
for section_directory in output/*/; do
    ...
    case "${section_number}" in
        ...
        *) section_title="LXsystem man pages"; echo "WARNING: Unrecognized section ${section_number}?!";;
    esac
...
pandoc -f gfm -t html5 -s \
    -B Meta/Websites/man.serenityos.org/banner-preamble.inc \
    --metadata title="LXsystem man pages" \
    -o output/index.html \
