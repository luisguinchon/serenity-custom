...
parted -s "${dev}" mklabel gpt mkpart EFI fat32 0% 512MB mkpart LXsystem ext2 512MB 100% set 1 esp on || die "couldn't partition disk"
echo "done"

printf "creating new filesystems... "
mkfs.vfat -F 32 -n EFI "${dev}p1" >/dev/null || die "couldn't create EFI system partition filesystem"
mke2fs -q -L LXsystem "${dev}p2" || die "couldn't create root filesystem"
echo "done"
